...
const MyDocument: React.FC<MyDocumentProps> = ({ pdfData }) => {
  ...
  const Header: React.FC = () => (
    <View style={styles.header}>
      <Image
        src="https://covalenseaccessibility.blob.core.windows.net/ai-portal/ai_portal_21b60868_test.png"
      ...
      <Page size="A4" style={styles.page}>
        ...
        {pdfData.map((item: any, i: any) => (
          <View style={[styles.section, styles.textOverflow]} key={i} >
            ...
            {item.answer.product_list && item.answer.product_list.map((productItem: any, index: any) => {
...
const DownloadPDF: React.FC<{ pdfData: MyDocumentProps[] }> = ({ pdfData }) => (
  <PDFDownloadLink style={{ display: "flex", alignItems: "center" }} document={<MyDocument pdfData={pdfData} />} fileName={`cglense_chat_${Date.now()}.pdf`}>
    {({ loading }) =>
      loading ? (
        'Loading'
      ) : (
